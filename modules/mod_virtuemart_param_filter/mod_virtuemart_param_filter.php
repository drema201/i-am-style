...
if($show_assing_filter){
	...
	if($customfields_show){
		...
		if($parent_auto && count($cids) > 0 && $mcf_mod_ajax){ // Автоматическая подборка полей + АЯКС = подгружать поля только из товаров отмеченных категорий
	...
	if(count($types) == 0){
		...
		$custom_int_count = getCustomfieldCount($types_ids,$param_search_ids,array(),'int',$children);
		// echo '<pre>'.print_r($custom_int_count,1).'</pre>';
		$custom_text_count = getCustomfieldCount($types_ids,$param_search_ids,array(),'text',$children);
	}
