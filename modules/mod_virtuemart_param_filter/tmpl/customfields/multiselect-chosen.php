...
if(!empty($customfield_value)){
	...
	$html .= '<select name="cv'.$type->virtuemart_custom_id.'[]" class="chosen" multiple style="width:100%;" data-placeholder="'.$mcf_customfields_select_heading.'">';
	...
	foreach($customfield_value as $v){
		...
		if($count_zero_show != 'hidden' || $count > 0){
			$html .= '<option value="'.$vid.'"'.$selected.'>'.$value.$count_txt.'</option>';
		}
	}
	$html .= '</select>';
	$html .= '</div>';
